...
contract OperatorContract {
    ...
    mapping (address => address) walletToContractAddrMap;
    address[] private usrWalletAddrList; 
    ...
    constructor(uint _ticketFee, uint _transitPassFee, uint _tariff, uint _tariffHigh) public {
        operatorWalletAddress = 0xC03651bc82064D114f6E9af588A362fE14FFf5A8; //hardcoded as its always same
    ...
    function setPassenger(address _contractAddr, address _ethWalletAddress, string memory _usrname) internal {
        Passenger memory newPassenger = passengers[_contractAddr];    
    
        newPassenger.username = _usrname;
    ...
    function generatePassengerContract(string memory _usrname) public {  
        ...
        setPassenger(address(newPassCon), msg.sender, _usrname);
        walletToContractAddrMap[msg.sender] = address(newPassCon);
    }
    ...
    function getUniquePassengerContractAddress() public view returns(address) {
        Passenger memory tempPass = passengers[walletToContractAddrMap[msg.sender]];
        return tempPass.ethWalletAddr;
    }
}
